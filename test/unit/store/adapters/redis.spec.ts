/**
 * Round-trip test for `src/store/adapters/redis.ts` against an in-memory
 * fake of an `ioredis` client — no real Redis connection.
 */

import { describe, expect, test } from 'bun:test';
import type { Redis } from 'ioredis';
import { RedisHashStore } from '../../../../src/store/adapters/redis.ts';

/** In-memory fake of an `ioredis` client, just enough surface (`get`/`set`) for the adapter. */
