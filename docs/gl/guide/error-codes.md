...
A AEAT publica o seu catálogo de erros como o ficheiro [`errores.properties`](https://prewww2.aeat.es/static_files/common/internet/dep/aplicaciones/es/aeat/tikeV1.0/cont/ws/errores.properties).
O SDK distribúe unha copia xerada en `src/errors/catalog.ts` (rexerada
con `bun run gen-errors` contra o ficheiro orixinal). Cada entrada leva:

- O `code` (cadea de 4 díxitos, chave primaria).
- A `category` — `'envelope'` (4xxx, rexeita o envío completo),
  `'record'` (1xxx / 3xxx, rexeita un rexistro) ou `'admissible'`
  (2xxx, acepta o rexistro con advertencia).
- O `message` literal en castelán tal e como o publica a AEAT.
- Unha explicación en inglés (`englishMessage`) engadida polo SDK para
  ergonomía do desenvolvedor. O texto en castelán é o autorizativo —
  móstrao ao usuario final cando o erro chegue a unha UI.
...
O catálogo é un obxecto `as const` conxelado, así que as chaves están
tipadas estaticamente e o teu IDE pode autocompletalas.

## Catálogo completo

> O texto en castelán mantense literal. Usa a mensaxe proporcionada pola
> AEAT en calquera UI orientada ao usuario para casar coa documentación
> que a AEAT publica para os contribuíntes.

| Código | Categoría | Mensaxe AEAT (castelán literal) | Explicación en inglés |
| ------ | --------- | ------------------------------- | ---------------------- |
...
- [`VerifactuError`](./validations.md) e as súas subclases — todo erro lanzado leva o código correspondente.
- [Validacións](./validations.md) — o validador local pre-asigna a cada incidencia de regra o código de catálogo máis próximo.
