...
The AEAT publishes its error catalog as the file [`errores.properties`](https://prewww2.aeat.es/static_files/common/internet/dep/aplicaciones/es/aeat/tikeV1.0/cont/ws/errores.properties).
The SDK ships a generated copy at `src/errors/catalog.ts` (regenerated by
`bun run gen-errors` against the upstream file). Every entry carries:

- The `code` (4-digit string, primary key).
- The `category` — `'envelope'` (4xxx, rejects the whole submission), `'record'` (1xxx / 3xxx, rejects one record) or `'admissible'` (2xxx, accepts the record with a warning).
- The verbatim Spanish `message` exactly as the AEAT publishes it.
- An English explanation (`englishMessage`) added by the SDK for developer
  ergonomics. The Spanish text is the authoritative one — show it to the user
  when the error reaches a UI.
...
The catalog is a frozen `as const` object, so the keys are statically typed
and your IDE can autocomplete them.

## Full catalog

> The Spanish text is kept verbatim. Use the AEAT-provided message in any
> user-facing UI to match the documentation the AEAT publishes for taxpayers.

| Code | Category | AEAT message (verbatim Spanish) | English explanation |
| ---- | -------- | ------------------------------- | ------------------- |
...
- [`VerifactuError`](./validations.md) and its subclasses — every thrown error carries the matching code.
- [Validations](./validations.md) — the local validator pre-maps each rule violation to the closest catalog entry.
