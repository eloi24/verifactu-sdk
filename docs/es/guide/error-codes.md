...
La AEAT publica su catálogo de errores como el fichero [`errores.properties`](https://prewww2.aeat.es/static_files/common/internet/dep/aplicaciones/es/aeat/tikeV1.0/cont/ws/errores.properties).
El SDK distribuye una copia generada en `src/errors/catalog.ts` (regenerada con
`bun run gen-errors` contra el fichero original). Cada entrada lleva:

- El `code` (cadena de 4 dígitos, clave primaria).
- La `category` — `'envelope'` (4xxx, rechaza el envío completo), `'record'`
  (1xxx / 3xxx, rechaza un registro) o `'admissible'` (2xxx, acepta el
  registro con advertencia).
- El `message` literal en español tal y como lo publica la AEAT.
- Una explicación en inglés (`englishMessage`) añadida por el SDK para
  ergonomía del desarrollador. El texto en español es el autoritativo —
  muéstralo al usuario final cuando el error llegue a una UI.
...
El catálogo es un objeto `as const` congelado, así que las claves están
tipadas estáticamente y tu IDE puede autocompletarlas.

## Catálogo completo

> El texto en español se mantiene literal. Usa el mensaje proporcionado por la
> AEAT en cualquier UI orientada al usuario para casar con la documentación que
> la AEAT publica para los contribuyentes.

| Código | Categoría | Mensaje AEAT (español literal) | Explicación en inglés |
| ------ | --------- | ------------------------------ | ---------------------- |
...
- [`VerifactuError`](./validations.md) y sus subclases — todo error lanzado lleva el código correspondiente.
- [Validaciones](./validations.md) — el validador local pre-asigna a cada incidencia de regla el código de catálogo más cercano.
